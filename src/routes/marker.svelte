<script>
    ...
    let { color, number, lngLat, task, mobilityType } = $props();
</script>

<Marker
    {lngLat}
    class="{color} grid h-10 w-10 place-items-center rounded-full border border-gray-200  text-black shadow-2xl focus:outline-2 focus:outline-black"
>
    <div class="relative">
        <span class="text-black pt-2 font-bold text-base">{number}</span>

        {#if mobilityType === "STUHL"}
            <svg
                class="top-0 left-3 transform -translate-y-3/4 absolute w-5 h-5 bg-blue-900 rounded-full text-white"
        ...
        {:else if mobilityType === "LIEGE"}
            <svg
                ...
                /></svg
            >
        {:else if mobilityType === "KEIN PATIENT"}
            <svg
                class="top-0 left-3 transform -translate-y-3/4 absolute w-5 h-5 bg-blue-900 rounded-full text-white"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 640 512"
