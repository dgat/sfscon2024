...
>
    ...
    <div class="flex items-center gap-2">
        ...
        <div class="breadcrumbs max-w-xs text-xs py-0">
            <ul>
                {#each transport.tasks as task (task.id)}
                    <li>{task.startPlace}</li>
                {/each}
            </ul>
        </div>
    </div>
    ...
    >
        <span>
            <svg
                ...
                /></svg
            >
            {transport.getLiegenCount()} / {transport.getMaxLiegenCount()}
        </span>
        <span>
            <svg
                class="size-4 inline-block"
                ...
                /></svg
            >
            {transport.getRollstuhleCount()} / {transport.getMaxRollstuhleCount()}
        </span>
        <span>
            <svg
                class="size-4 inline-block"
                ...
                /></svg
            >
            {transport.getGehenCount()} / {transport.getMaxGehenCount()}
        </span>
    </div>
</div>
